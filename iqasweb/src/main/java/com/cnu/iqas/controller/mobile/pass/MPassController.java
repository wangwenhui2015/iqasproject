...
import com.lyt.TestFourGroup.TestFourGroupImpl;
import com.lyt.UpdateLearnAction.UpdateLearnAction;
import com.lyt.UpdateLearnAction.UpdateLearnActionImpl;
import com.lyt.entity.LearnTest;
...
public class MPassController {
	...
	@RequestMapping(value = "wordAndSentencesByGrade")
	public ModelAndView wordAndSentencesByGrade(String grade) {
		System.out.println("进入wordAndSentencesByGrade");
		ModelAndView mv = new ModelAndView("share/json");
		MyStatus status = new MyStatus();
		// 总的Json对象
		JSONObject jsonObject = new JSONObject();
		JSONArray usersArray = new JSONArray();
		JSONObject wordandsentence = new JSONObject();
		try {
			...
			for (int wordIndex = 0; wordIndex < resultsFiveWordsOfThisGrade.size(); wordIndex++) {
				while (resultsFiveWordsOfThisGrade.get(wordIndex).hasNext()) {
					...
					System.out.println("\n");
					wordandsentence.put("word", word);
					usersArray.add(wordandsentence);
				}
			}
		} catch (Exception e) {
	...
	// 该方法用于前测模块的返回数据，数据包括金币数量和场景。方法的参数是用户名、金币数、场景
		...
		public ModelAndView listTestWordattribute(int num) {
			...
			try {
				...
				System.out.println("生成的测试四元组为：" + lt.getTestKnowledgeId() + "/" + lt.getTestType() + "/" + lt.getTestAspect() + "/" + lt.getTestDifficulty());
	...
	public ModelAndView saveMiddleTest(String UserId,String userName, String TestKnowledgeId, int TestType, int TestAspect,
			int TestDifficulty, int Pass,int tempcoin) {
		...
		try {
		  ...
		  //MiddleTest.put("Pass", Pass);
		//	MiddleTest.put("date", date);
			usersArray.add(MiddleTest);
		} catch (Exception e) {
			status.setMessage("未知异常");
			status.setStatus(StatusConstant.UNKONWN_EXECPTION);
