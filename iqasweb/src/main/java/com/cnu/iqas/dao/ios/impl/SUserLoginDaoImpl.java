...
public class SUserLoginDaoImpl extends DaoSupport<SuserLogin> implements SUserLoginDao {
	...
	public SuserLogin findCurrentLogin(String userId, String ip) {
		List<SuserLogin> list =getHt().find(" From SuserLogin o where o.userId=? and ip =? and loginState=1", userId,ip);
	...
	public List<SuserLogin> findLogining(String userId) {
		List<SuserLogin> list =getHt().find(" From SuserLogin o where o.userId=? and loginState=1", userId);
		return list;
	}
