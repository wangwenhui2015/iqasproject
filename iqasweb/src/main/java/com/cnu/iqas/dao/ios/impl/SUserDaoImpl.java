...
public class SUserDaoImpl  extends DaoSupport<Suser> implements SUserDao {
	...
	public Suser findByNameAndPas(String username, String password) {
		...
		List<Suser> users =getHt().find("From Suser o where  o.userName=? and  o.password=?", username,password);
	...
	public Suser findByName(String userName) {
		List<Suser> users =getHt().find("From Suser o where  o.userName=?", userName);
		if(users!=null&&users.size()==1)
			return users.get(0);
		return null;
	}
