...
public class FriendRequestDaoImpl extends DaoSupport<FriendRequest>implements FriendRequestDao {
	...
	public FriendRequest findRequest(String ownUserName, String friendUserName) {
		List<FriendRequest> list= getHt().find("From FriendRequest o where o.ownUserName=? and o.friendUserName=? ", ownUserName,friendUserName);
	...
	public List<FriendRequest> fetchRequests(String userName) {
		List<FriendRequest> list= getHt().find("From FriendRequest o where o.friendUserName=? and isHandle = ?",userName,false);
