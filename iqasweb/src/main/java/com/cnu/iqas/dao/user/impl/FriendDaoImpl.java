...
public class FriendDaoImpl extends DaoSupport<Friend>implements FriendDao {
	...
	public Friend findFriendRel(String oneUserName, String otherUserName) {
		...
		List<Friend> list =getHt().find("From Friend o where o.oneUserName=? and o.otherUserName=?", oneUserName,otherUserName);
		
		Friend friend= (list!=null && list.size()>0)?list.get(0):null;
		//如果没找到，在调换位置找
		if( friend ==null){
		  list =getHt().find("From Friend o where o.oneUserName=? and o.otherUserName=?",otherUserName ,oneUserName);
